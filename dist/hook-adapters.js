...
const CONTEXT_HEADER = 'kodemux routing recommendation (deterministic — verify against your own read of the repo before deferring to it):';
/**
 ...
 */
export async function userPromptSubmitHook(payload) {
    ...
    lines.push(decision.target && decision.target.mode === 'multi-agent'
    ...
    if (decision.confidence < 0.6) {
        lines.push(`  confidence ${decision.confidence} — deterministic signal is thin here; use your own read of the repo to confirm or override this tier.`);
    }
 ...
 */
export async function preToolUseHook(payload) {
    const cwd = payload.cwd ?? process.cwd();
...
//# sourceMappingURL=hook-adapters.js.map