...
export type JudgeOutcome = { ok: true; result: JudgeResult } | { ok: false; reason: string };

const KNOWN_RISKS: RiskFlag[] = ['security', 'production', 'critical'];

function isRiskFlag(v: unknown): v is RiskFlag {
  return typeof v === 'string' && (KNOWN_RISKS as string[]).includes(v);
}
...
function buildJudgePrompt(prompt: string, signals: Signals): string {
  ...
  if (signals.fileCount !== undefined) lines.push(`Files changed: ${signals.fileCount}`);
  if (signals.diffLines !== undefined) lines.push(`Diff size: ~${signals.diffLines} lines`);
  if (signals.paths?.length) lines.push(`Changed paths: ${signals.paths.slice(0, 20).join(', ')}`);
  lines.push(
    '',
...
export interface MinimalAnthropicClient {
  messages: {
    create(params: unknown, options?: { timeout?: number }): Promise<{
      stop_reason?: string;
 ...
 */
export async function judgeWithClient(
 ...
 * to `{ ok: false, reason }`.
 */
export async function judgeComplexity(prompt: string, signals: Signals = {}): Promise<JudgeOutcome> {
