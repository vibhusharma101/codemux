...
export function kodemuxBlock(): string {
  return [
    BEGIN_MARKER,
...
function targetPath(cwd: string, opts: CodexInstallOptions): string {
 ...
 */
export function upsertBlock(existing: string, block: string): string | null {
...
export function installCodex(cwd: string, opts: CodexInstallOptions = {}): CodexInstallOutput {
  ...
  if (existing) writeFileSync(`${path}.bak`, existing, 'utf8');
  ...
  if (existing) lines.push(`  backup: ${path}.bak`);
